...
class Robot(models.Model):
    ...
    # Updated the on_delete behavior to SET_NULL
    ...
    is_purchased = models.BigIntegerField(default=False)
...
class IPAddress(models.Model):
    robot = models.ForeignKey(Robot, on_delete=models.CASCADE, related_name='ip_addresses')
    ip_address = models.CharField(max_length=255,null=True,blank=True)
    created_at = models.DateTimeField(auto_now_add=True)  # Timestamp when added

    def __str__(self):
        return f"{self.robot.robo_name} - {self.ip_address}"
    

class RobotStop(models.Model):
    robot = models.ForeignKey(
        'Robot',
        on_delete=models.CASCADE,
        related_name='stops'
    )
    status = models.BooleanField(default=False)  # True = stopped, False = running
    stopped_at = models.DateTimeField(auto_now_add=True)  # Optional timestamp

    def __str__(self):
        return f"{self.robot.robo_name} - {'Stopped' if self.status else 'Running'}"
    

class RobotStatus(models.Model):
    robot=models.ForeignKey('Robot',on_delete=models.CASCADE,related_name='robot_status')
    status=models.BooleanField(default=True)
    offline=models.DateTimeField(auto_now_add=True)
