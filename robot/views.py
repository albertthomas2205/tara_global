...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def list_robot_pdfs(request, robo_id):
    """
    List all PDF files for the specified `robo_id`.
    """
    directory_path = os.path.join(settings.MEDIA_ROOT, 'PDF', robo_id)

    if not os.path.exists(directory_path):
        return Response(
            {"status": "ok", "pdfs": [], "message": "No PDFs found."},
            status=status.HTTP_200_OK,
        )

    pdf_files = [
        request.build_absolute_uri(
            os.path.join(settings.MEDIA_URL, 'PDF', robo_id, f).replace('\\', '/')
        )
...
def get_robot_pdf_details(request, robo_id):
    ...
    Returns the file URL if the PDF exists and is not older than 20 seconds.
    """
    directory_path = os.path.join(settings.MEDIA_ROOT, 'PDF', robo_id)
    if not os.path.exists(directory_path):
        return Response(
    ...
    # Get all PDF files sorted by modification time (latest first)
    ...
    # Get the latest file and check its age
    latest_pdf = pdf_files[0]
    file_path = os.path.join(directory_path, latest_pdf)
    file_mod_time = os.path.getmtime(file_path)
    current_time = time.time()

    # If file is older than 20 seconds, return no file found
    if current_time - file_mod_time > 20:
        return Response(
            {"status": "error", "message": "No PDF found for the given robo_id (expired)."},
            status=status.HTTP_404_NOT_FOUND,
        )

    # Generate URL
    relative_file_url = os.path.join(settings.MEDIA_URL, 'PDF', robo_id, latest_pdf).replace('\\', '/')
    absolute_file_url = request.build_absolute_uri(relative_file_url)
...
@api_view(['POST'])
def add_ip_address(request, robo_id):  # Use 'robo_id' instead of 'robot_id'
    """Add or update the IP address for a given robot ID."""
    ...
    try:
        robot = Robot.objects.get(robo_id=robo_id)  # Match field name
    except Robot.DoesNotExist:
    ...
    serializer = IPAddressSerializer(obj)
    return Response({"message":"ip added successfully","data":serializer.data}, status=status.HTTP_201_CREATED if created else status.HTTP_200_OK)




@api_view(['GET'])
def get_last_ip(request, robo_id):  # Use 'robo_id' to match URL
    """Retrieve the last added IP address for a given robot ID."""
    try:
        robot = Robot.objects.get(robo_id=robo_id)
    except Robot.DoesNotExist:
        return Response({"error": "Robot not found"}, status=status.HTTP_404_NOT_FOUND)

    last_ip = IPAddress.objects.filter(robot=robot).order_by('-created_at').first()

    if last_ip:
        serializer = IPAddressSerializer(last_ip)
        return Response({"status":"OK" ,"message":"ip retrieved successfully","data":serializer.data}, status=status.HTTP_200_OK)
    else:
        return Response({"error": "No IP address found for this robot"}, status=status.HTTP_404_NOT_FOUND)
    

@api_view(['POST'])
def stop_robot(request, robo_id):
    """Button 1 click → Stop robot (status=True)"""
    try:
        robot = Robot.objects.get(robo_id=robo_id)
    except Robot.DoesNotExist:
        return Response({'error': 'Robot not found'}, status=status.HTTP_404_NOT_FOUND)
...
@api_view(['POST'])
def start_robot(request, robo_id):
    """Button 2 click → Start robot (status=False)"""
    try:
        robot = Robot.objects.get(robo_id=robo_id)
    except Robot.DoesNotExist:
        return Response({'error': 'Robot not found'}, status=status.HTTP_404_NOT_FOUND)

    # Find latest stop record and update it, or create a new one
    stop_record = RobotStop.objects.filter(robot=robot).order_by('-stopped_at').first()
    if stop_record:
        stop_record.status = False
        stop_record.save()
    else:
...
@api_view(['GET'])
def get_current_robot_status(request, robo_id):
    """Get the latest stop/start status of the robot."""
    try:
        robot = Robot.objects.get(robo_id=robo_id)
    except Robot.DoesNotExist:
        return Response({'error': 'Robot not found'}, status=status.HTTP_404_NOT_FOUND)

    latest_stop = RobotStop.objects.filter(robot=robot).order_by('-stopped_at').first()

    if latest_stop:
        return Response({
            'robot': robot.robo_name,
            'robo_id': robot.robo_id,
            'status': latest_stop.status,
            'stopped_at': latest_stop.stopped_at
        }, status=status.HTTP_200_OK)
    ...
    return Response({
        ...
        'robo_id': robot.robo_id,
        'status': None,
        'message': 'No stop/start status recorded yet'
    }, status=status.HTTP_200_OK)