...
# Helper to get the robot-specific upload path
def get_robot_upload_dir(robot_id):
    upload_dir = os.path.join(settings.MEDIA_ROOT, 'stm_files', robot_id)
    os.makedirs(upload_dir, exist_ok=True)
    return upload_dir

@api_view(['POST'])
def upload_stcm_file(request, robot_id):
    ...
    if serializer.is_valid():
        ...
        robot_dir = os.path.join(settings.MEDIA_ROOT, 'stm_files', robot_id)
...
@api_view(['GET'])
def get_latest_stcm_file(request, robot_id):
    """Get the latest uploaded .stcm file for a robot_id"""
    try:
        upload_dir = get_robot_upload_dir(robot_id)
        ...
        latest_file = files[0]
        file_url = request.build_absolute_uri(os.path.join(settings.MEDIA_URL, 'stm_files', robot_id, latest_file))

        return Response({"latest_file": latest_file, "robot_id": robot_id, "file_url": file_url})
...
@api_view(['DELETE'])
def delete_stcm_file(request, robot_id):
    ...
    try:
        ...
        return Response({"message": "File deleted", "robot_id": robot_id}, status=status.HTTP_200_OK)
...
def create_youtube(request):
    ...
    if serializer.is_valid():
        serializer.save()
        return Response({"status":"ok","message":"video added successfully","data":serializer.data}, status=status.HTTP_201_CREATED)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)


@api_view(['GET'])
@permission_classes([IsAuthenticated])
def list_youtube(request):
